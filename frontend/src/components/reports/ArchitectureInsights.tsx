...
export function ArchitectureInsights({ insight }: ArchitectureInsightsProps) {
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-900">Architecture Review</h2>

      {/* Pattern Justification */}
      <div className="bg-white rounded-2xl p-6 border border-indigo-200 shadow-sm">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">🏛️ Primary Pattern</h3>
        <div className="bg-gray-50 rounded-lg p-4 mb-4">
          <p className="text-2xl font-bold text-indigo-600 mb-2">
            {insight.pattern_justification.primary_pattern}
          </p>
          <p className="text-gray-700">{insight.pattern_justification.confidence_explanation}</p>
        </div>
        ...
        <div className="space-y-3">
          ...
          {insight.pattern_justification.alternative_patterns_considered.length > 0 && (
            <div>
              <p className="text-sm font-semibold text-gray-700 mb-2">Alternatives Considered:</p>
      ...
      <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            ...
            <p className="text-2xl font-bold text-gray-900">{insight.quality_assessment.overall_rating}</p>
          </div>
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <p className="text-sm text-gray-600 mb-1">Layering Score</p>
            <p className="text-2xl font-bold text-blue-600">{insight.quality_assessment.layering_score}/100</p>
          </div>
        </div>
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            <span className="text-gray-600">Dependency Management:</span>
            <span className="font-medium text-gray-900">{insight.quality_assessment.dependency_management}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">Scalability Potential:</span>
            <span className="font-medium text-gray-900">{insight.quality_assessment.scalability_potential}</span>
          </div>
        </div>
      </div>
            ...
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex gap-2 ml-4">
                  <span className="text-xs px-2 py-1 rounded bg-white whitespace-nowrap">
                    {rec.priority}
                  </span>
                  <span className="text-xs px-2 py-1 rounded bg-white whitespace-nowrap">
                    {rec.effort} effort
              ...
              <div className="bg-white rounded p-3 mt-3">
                <p className="text-xs font-semibold text-gray-700 mb-1">Implementation Guide:</p>
                <p className="text-xs text-gray-600">{rec.implementation_guide}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Tech Stack Analysis */}
      <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">🔧 Tech Stack Analysis</h3>
        <p className="text-gray-700 mb-4">{insight.tech_stack_analysis.framework_fit}</p>

        {insight.tech_stack_analysis.missing_practices.length > 0 && (
          <div className="mb-4">
            <h4 className="font-semibold text-gray-900 mb-2">Missing Practices:</h4>
            <div className="flex flex-wrap gap-2">
        ...
        {insight.tech_stack_analysis.modernization_suggestions.length > 0 && (
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">Modernization Suggestions:</h4>
            ...
            {insight.real_world_examples.map((example, idx) => (
              <div key={idx} className="bg-gray-50 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-semibold text-gray-900">{example.project}</h4>
                  <a
                    href={example.url}
                    target="_blank"
                ...
                <p className="text-sm text-gray-600">{example.relevance}</p>
              </div>
            ))}
          </div>
        </div>
