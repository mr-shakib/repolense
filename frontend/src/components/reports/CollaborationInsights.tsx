  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 border border-teal-200 shadow-sm">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">📊 Collaboration Score</h3>
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-4xl font-bold text-teal-600">{insight.collaboration_score_breakdown.current_score}</p>
            <p className="text-sm text-gray-600">Current Score</p>
          </div>
          <div className="text-right">
            <p className="text-2xl font-bold text-gray-900">{insight.collaboration_score_breakdown.realistic_improvement_target}</p>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {insight.collaboration_score_breakdown.factors_raising_score.length > 0 && (
            <div className="bg-white rounded-2xl p-4">
              <h4 className="font-semibold text-green-900 mb-2">Positive Factors</h4>
      ...
      <div className="bg-white rounded-2xl p-6 border border-gray-200">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-4">
          <div className="p-3 bg-gray-50 rounded">
            ...
            <p className="font-semibold text-gray-900">{insight.team_dynamics.collaboration_health}</p>
          </div>
          <div className="p-3 bg-gray-50 rounded">
            <p className="text-xs text-gray-600 mb-1">Diversity</p>
            <p className="font-semibold text-gray-900">{insight.team_dynamics.contributor_diversity}</p>
          </div>
          <div className="p-3 bg-gray-50 rounded">
            <p className="text-xs text-gray-600 mb-1">Communication</p>
            <p className="font-semibold text-gray-900">{insight.team_dynamics.communication_patterns}</p>
          </div>
        </div>
        ...
        {insight.team_dynamics.potential_concerns.length > 0 && (
          <div className="bg-yellow-50 rounded p-3">
            <h4 className="font-semibold text-yellow-900 mb-2">⚠️ Concerns:</h4>
            <ul className="space-y-1">
      ...
      <div className="bg-white rounded-2xl p-6 border border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">🧠 Knowledge Distribution</h3>
        
        <div className="bg-red-50 rounded-2xl p-4 mb-4">
          <div className="flex items-center justify-between mb-2">
            <span className="font-semibold text-gray-900">Bus Factor Risk</span>
        ...
        {insight.knowledge_distribution.critical_areas.length > 0 && (
          <div className="space-y-3 mb-4">
            <h4 className="font-semibold text-gray-900">Critical Areas:</h4>
            {insight.knowledge_distribution.critical_areas.map((area, idx) => (
              <div key={idx} className={`p-4 rounded-2xl border-l-4 ${
                ...
                <p className="text-sm text-gray-600 mb-1">Owner: {area.primary_owner}</p>
                <p className="text-sm text-blue-600">💡 {area.mitigation}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {insight.knowledge_distribution.cross_training_needs.length > 0 && (
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">Cross-Training Needs:</h4>
      ...
      {insight.team_bottlenecks.length > 0 && (
        <div className="bg-white rounded-2xl p-6 border border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">🚧 Team Bottlenecks</h3>
          <div className="space-y-3">
            {insight.team_bottlenecks.map((bottleneck, idx) => (
              <div key={idx} className={`p-4 rounded-2xl ${
                ...
                <div className="flex items-center justify-between mb-2">
                  <span className="font-semibold text-gray-900">{bottleneck.bottleneck}</span>
                  <span className="text-xs px-2 py-1 rounded bg-white">
                    {bottleneck.severity}
                  </span>
                </div>
                <p className="text-sm text-gray-700 mb-2">{bottleneck.impact}</p>
                <p className="text-sm text-blue-600">💡 Solution: {bottleneck.solution}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-green-200 shadow-sm">
        ...
        <div className="bg-white rounded-2xl p-4 mb-4">
          <p className="text-sm font-semibold text-gray-700 mb-2">Team Readiness:</p>
          <p className="text-lg font-bold text-gray-900">{insight.hiring_insights.team_readiness}</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          {insight.hiring_insights.candidate_fit_criteria.length > 0 && (
            <div>
              <h4 className="font-semibold text-gray-900 mb-2">Candidate Fit:</h4>
              <ul className="space-y-1">
          ...
          {insight.team_health_recommendations.map((rec, idx) => (
            <div key={idx} className={`p-4 rounded-2xl border-l-4 ${
              ...
              <div className="flex items-start justify-between mb-2">
                <span className="font-semibold text-gray-900">{rec.recommendation}</span>
                <span className="text-xs px-2 py-1 rounded bg-white whitespace-nowrap ml-2">
                  {rec.priority}
                </span>
              </div>
              <p className="text-sm text-gray-600 mb-2">Category: {rec.category}</p>
              <p className="text-sm text-gray-700 mb-2">{rec.implementation}</p>
              <p className="text-sm text-blue-600">✨ {rec.expected_benefit}</p>
            </div>
          ))}
        </div>
      </div>
