      ...
      <div className="space-y-6">
        ...
        {rawData.issues && rawData.issues.length > 0 && (
          <div className="bg-red-50 rounded-2xl p-6">
            <h3 className="text-lg font-semibold text-red-900 mb-3">⚠️ Issues</h3>
            <ul className="space-y-2">
        ...
        {rawData.file_metrics && (
          <div className="bg-white rounded-2xl p-6 border">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">📊 File Metrics</h3>
            <dl className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <dt className="text-gray-600">Total Files</dt>
                <dd className="font-semibold text-gray-900">{rawData.file_metrics.total_files}</dd>
              </div>
              <div>
                <dt className="text-gray-600">Total Lines</dt>
                <dd className="font-semibold text-gray-900">{rawData.file_metrics.total_lines}</dd>
              </div>
              <div>
                <dt className="text-gray-600">Avg File Length</dt>
                <dd className="font-semibold text-gray-900">{rawData.file_metrics.avg_file_length?.toFixed(1)}</dd>
              </div>
              <div>
                <dt className="text-gray-600">Max File Length</dt>
                <dd className="font-semibold text-gray-900">{rawData.file_metrics.max_file_length}</dd>
              </div>
            </dl>
          </div>
        )}
      ...
      <div className="space-y-6">
        ...
        {rawData.principle_score !== undefined && (
          <div className="bg-white p-6 rounded-2xl border">
            ...
            {rawData.grade && (
              <div className="mt-2 text-gray-600">Grade: {rawData.grade}</div>
            )}
          </div>
        )}
            ...
            <div className="space-y-3">
              {rawData.violations.slice(0, 10).map((violation: any, idx: number) => (
                <div key={idx} className="border-l-4 border-red-500 pl-4 py-2">
                  <div className="font-semibold text-gray-900">{violation.principle}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {rawData.bus_factor !== undefined && (
            <div className="bg-white p-4 rounded-2xl border">
              ...
              <div className="text-2xl font-bold text-gray-900">{rawData.bus_factor}</div>
            </div>
          )}
          {rawData.active_contributors_count !== undefined && (
            <div className="bg-white p-4 rounded-2xl border">
              <div className="text-sm text-gray-600">Active Contributors</div>
              <div className="text-2xl font-bold text-gray-900">{rawData.active_contributors_count}</div>
            </div>
          )}
          {rawData.collaboration_score !== undefined && (
            <div className="bg-white p-4 rounded-2xl border">
              <div className="text-sm text-gray-600">Collaboration Score</div>
              <div className="text-2xl font-bold text-gray-900">{rawData.collaboration_score.toFixed(1)}</div>
            </div>
          )}
        </div>
            ...
            <div className="space-y-3">
              {rawData.contributors.slice(0, 5).map((contributor: any, idx: number) => (
                <div key={idx} className="flex items-center justify-between">
                  <div>
                    <div className="font-semibold text-gray-900">{contributor.username}</div>
